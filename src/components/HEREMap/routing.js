import { mapElements,getDistrict,getMCPbyDistrict,getMCPbyDistrictName,getFactorybyDistrictName, mapScale } from "./MapElement.js";
...
export function createPackageRoutingContent(origin,dest){
    var routing_param={};
...
export function addRoutes(contentPackage=routingContentPackage){
...
export var location_to_route=[];
export var temp_route=[];
var startPoint=0;
var endPoint=0;
//Add event for button
export var stateEditRouting={
    Enable:1,
    Disable:0
}
export var state_edit=stateEditRouting.Disable;
export function ActivateEditRoute(){
    state_edit=stateEditRouting.Enable;
}
export function DisableEditRoute(){
    state_edit=stateEditRouting.Disable;
}
export function AddStartPoint(marker){
    startPoint=marker;
}
export function AddEndPoint(marker){
    endPoint=marker;
}
    ...
    if(temp_route.length>0){
        ...
        for(let i=0;i<location_to_route.length;i++){
            for(let j=0;j<mcps.length+1;j++){
                if(j==mcps.length){
                    if(this_district.factory.location.lat==location_to_route[i].lat && this_district.factory.location.lng==location_to_route[i].lng){
                        this_district.routeInfo.location.push({name:this_district.factory.name, 
                            address:this_district.factory.address, location:{lat:this_district.factory.location.lat, lng:this_district.factory.location.lng}});
                        break;
                    }
                }else{
                    if(mcps[j].location.lat==location_to_route[i].lat && mcps[j].location.lng==location_to_route[i].lng){
                        console.log("current"+String(i));
                        this_district.routeInfo.location.push({name:mcps[j].name,
                            address:mcps[j].address, location:{lat:location_to_route[i].lat, lng:location_to_route[i].lng}});
                    }
                }
            }
        }
        ...
        if("routeInfo" in this_district)
            delete this_district.routeInfo;
    }
...
function UndoRouting() {
    ...
    if(length>2){
        map.removeObject(endPoint);  //remove current endPoint
        endPoint=AddMarker(map,location_to_route[length-2],markerType.endPoint);  //add new endpoint
    }else if(length==2){  //remove the last endpoint
        map.removeObject(endPoint);  //remove current endPoint
        endPoint=0;
    }else if(length==1){  //there is startPoint only
        map.removeObject(startPoint);
        startPoint=0;
    }
    if(length>0)
        location_to_route.pop()
}
...
function ClearRoutes(){
    ...
    for(let i=0;i<length;i++){
        ...
        if(data!=markerType.MCP && data!=markerType.Factory){ //delete all except MCP and Factory
...
function ClearRoutingBuffer(){
    ...
    for(let i=0;i<length;i++){ //clear routes
        var route=temp_route.pop();
        map.removeObjects([route.outline,route.arrow]);
    }
...
function OptimizeRoutes(){
    ...
    // console.log("click");
    var content=JSON.stringify(constructTSPRequest(getMCPbyDistrictName(current_focus.name),getDistrict(current_focus.name).factory));
    req.send(content);
    var result=JSON.parse(req.responseText);
    var stops=result.tours[0].stops;
    for(let i=0;i<stops.length;i++){  //add to buffer location_to_route
        var stop=stops[i];
    ...
    for(let i=0;i<location_to_route.length-1;i++){  //from location_to_route buffer, draw
        createPackageRoutingContent(location_to_route[i],location_to_route[i+1]);
    }
    addRoutes();
}
