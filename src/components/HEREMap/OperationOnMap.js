...
import { getMCPbyDistrict,getMCPbyDistrictName, mapElements, getMCPbyLocation } from "./MapElement.js";
import { location_to_route, stateEditRouting,state_edit,createPackageRoutingContent,addRoutes,AddStartPoint, getEndPoint, AddEndPoint } from "./routing.js";
import { current_focus } from "./searh-module.js";
...
//Location to be the center on map
export function focusLocation(map, region){
...
function removeElement(array, marker){
    ...
    for(let i=0;i<array.length;i++){
        if(array[i]==marker){
            found=true;
            for(let z=i;z<array.length-1;z++){ //loop to shift
                array[i]=array[i+1];
            }
        }
    }
    if(found) //if marker is found, reduce array length
        array.pop();
    return found;
}
function MapOnClick(evt){
    ...
    switch(tapStatus){
        case stateEdit.ModifyNoMCP:
            if(actionType==OperationTypeOnTap.Add){ //perform add
                var marker=AddMarker(map, coords,markerType.MCP,MCP_marker_size);
        ...
        case stateEdit.ModifyNoFactory:
            if(actionType==OperationTypeOnTap.Add){  //perform add
                var marker=AddMarker(map,coords,markerType.Factory,Factory_marker_size);
                marker.addEventListener('tap',performClickMarker);
                temp_Marker_Buffer_to_delete.push(marker);
            }
            break;
...
//Create marker
    //Create icon for marker
    export function CreateIcon(path_to_icon,size=0){
    ...
    //Add marker
    export function AddMarker(this_map,location, type,size=0){
        var path_to_icon="";
        if(type==markerType.MCP){
            path_to_icon='images/Landfill.png';
        ...
        var marker=new H.map.DomMarker(location, {icon:icon, data:type, volativity: false});
        if(type==markerType.MCP){
    ...
    export function increaseOpacity(evt){evt.target.style.opacity=0.8;}
    export function reduceOpacity(evt){evt.target.style.opacity=1;}

    function performClickMarker(evt){
        var markertype=this.getData();
        if(tapStatus==stateEdit.ModifyNoMCP && markertype==markerType.MCP){
            if(actionType==OperationTypeOnTap.Delete){  //perform delete
                var marker_existed=  removeElement(temp_Marker_Buffer_to_delete,this);
                if(!marker_existed){
                    temp_Marker_Buffer_to_add.push(this);
                }
                RemvoveMarker(map,this);
            }   
        }else if(tapStatus==stateEdit.ModifyNoFactory && markertype==markerType.Factory){
            if(actionType==OperationTypeOnTap.Delete){  //perform delete
                var marker_existed=  removeElement(temp_Marker_Buffer_to_delete,this);
                if(!marker_existed){
                    temp_Marker_Buffer_to_add.push(this);
        ...
        if(state_edit==stateEditRouting.Enable){
            var this_location={lat: this.getGeometry().lat, lng: this.getGeometry().lng};
            ...
            if(length>=2){  //create route
                createPackageRoutingContent(location_to_route[length-2],location_to_route[length-1]);
                addRoutes();
                //Remove previous endPoint first
                var pre_endPoint=getEndPoint();
                if(pre_endPoint!=0)
                    map.removeObject(pre_endPoint);
                //Add new endPoint
                var marker=AddMarker(map,this_location,markerType.endPoint);
                marker.addEventListener('tap',performClickMarker);
                AddEndPoint(marker); //define temporary endPoint
            }else if(length==1){  //Add marker for start point
...
//Add event to buttons
    ...
    function RunMCP(){
        if(tapStatus!=stateEdit.Disable && tapStatus!=stateEdit.ModifyNoMCP){
    ...
    function RunFactory(){
        if(tapStatus!=stateEdit.Disable && tapStatus!=stateEdit.ModifyNoFactory){
            alert("Finish current action first");
            return;
        }
...
export function ViewMapLocations(district_region){
    //Add marker for factory
    if("factory" in district_region){
        var factory=district_region.factory;
        var marker=AddMarker(map,factory.location,markerType.Factory,Factory_marker_size);
        marker.addEventListener('tap',performClickMarker);
    }
    var mcps=getMCPbyDistrict(district_region);
    for(let i=0;i<mcps.length;i++){
        var marker=AddMarker(map,mcps[i].location,markerType.MCP,MCP_marker_size);
        marker.addEventListener('tap',performClickMarker);
    }
}
export function ShowRoutes(district){
    ...
    var start_point=AddMarker(map,district.routeInfo.location[0].location,markerType.startPoint);
    var end_point=AddMarker(map,district.routeInfo.location[district.routeInfo.location.length-1].location,markerType.endPoint);
    map.addObjects([start_point,end_point]);
}

// var MCP1=AddMarker(map,{lat:10.7557,lng:106.66398},markerType.MCP,MCP_marker_size); //124 Đường Hồng Bàng, Phường 12 Quận 5, chốt
// var MCP2=AddMarker(map,{lat:10.75643,lng:106.67316},markerType.MCP,MCP_marker_size); // 292 Trần Phú, Phường 8 Quận 5, chốt
// var MCP3=AddMarker(map,{lat:10.75821,lng:106.66035},markerType.MCP,MCP_marker_size); // 201A Nguyễn Chí Thanh, Phường 12, Quận 5, chốt
// var MCP4=AddMarker(map,{lat:10.75634,lng:106.66786},markerType.MCP,MCP_marker_size); // Công viên Văn Lang, Phường 9 Quận 5, chốt
// var MCP5=AddMarker(map,{lat:10.75933,lng:106.66599},markerType.MCP,MCP_marker_size);// 153 Nguyễn Chí Thanh, Phường 9 Quận 5, chốt
// var MCP6=AddMarker(map,{lat:10.75284,lng:106.67116},markerType.MCP,MCP_marker_size);// 7 Trần Hưng Đạo B, Phường 6 Quận 5, chốt
// var Factor1=AddMarker(map,{lat:10.76077,lng:106.68231},markerType.Factory,Factory_marker_size); //280 An Dương Vương, Phường 4 Quận 5, chốt
